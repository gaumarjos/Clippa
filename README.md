...
2. Download one file manually and copy your authentication cookies from the request starting
   with `get-download-url/?...` > Request Headers > Cookie and store it in the file `cookies.txt`. Cookies have
...
2. Run `rippamanifest.py` to download all clip manifests and generate a `manifest.json`. Edit the output if not all videos
   are needed.
...
## Download recordings of live lessons
1. Get the JSON
2. In it, find the `streamingUrl` of the the video you want to download

![](docs/recordings.png)

3. Set and run `./rippalive.sh <streamingUrl> [output_name] [seconds]`